...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/?error=hubspot_auth_failed`)
  }
  ...
  try {
    const tokenResponse = await fetch('https://api.hubapi.com/oauth/v1/token', {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/auth/hubspot/callback`,
        code,
      }),
    })
    ...
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/?success=hubspot_connected`)
