...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/?error=mailchimp_auth_failed`)
  }
  ...
  try {
    const tokenResponse = await fetch('https://login.mailchimp.com/oauth2/token', {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/auth/mailchimp/callback`,
        code,
      }),
    })
    ...
    if (tokens.error) {
      console.error('Mailchimp OAuth error:', tokens)
      return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/?error=token_exchange_failed`)
    }
    
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/?success=mailchimp_connected`)
